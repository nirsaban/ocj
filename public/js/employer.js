...
function sortJobs(data) {
      ...
      const allJobs = document.querySelectorAll('#courseName');
      allJobs.forEach(job =>{
          if(job.textContent.trim() !== data.options[data.selectedIndex].text.trim()){
             document.getElementById(`${job.textContent.trim().replace(' ','')}`).style.display ='none'
          }
      })
